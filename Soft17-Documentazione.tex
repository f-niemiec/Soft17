...
\subsection{Overview}
Abbiamo deciso di sfruttare l'ambiente condiviso online \textbf{colab} e di conseguenza il linguaggio scelto per raggiungere l'obiettivo prefissato è \textbf{python}. In particolare abbiamo utilizzato le seguenti librerie: 
\begin{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Double e Split} non saranno considerate opzioni tra le azioni attuabili per semplificare il processo decisionale dell'agente.
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \item \textbf{Statico}: l’ambiente non cambia mentre l’agente sta delrando;
...
\begin{itemize}
    ...
    \item \textbf{player\_pair}: vaCriabile binaria che indica se la mano è composta da due carte dello stesso valore e quindi se è possibile effettuare lo \emph{split}.
\end{itemize}
...
\end{itemize}
  ...
  \begin{center}
      \includegraphics[scale=1.0, center]{ConfusionMatrixPipeline1A.png}
  \end{center}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{SARSApolicy.png}
    \caption{SARSA Policy – Online SARSA}
    \label{fig:SARSAonlineResult2}
\end{figure}

\subsection{Implementazione Q-Learning Online}
Il Q-Learning online rappresenta il primo algoritmo \textit{off-policy}
implementato nell’ambito del progetto. Dopo aver analizzato un approccio
\textit{on-policy} tramite SARSA, l’obiettivo di questa fase è valutare
come la separazione tra \textit{behavior policy} e \textit{target policy}
influenzi il processo di apprendimento e il comportamento finale
dell’agente.

A differenza di SARSA, che aggiorna i valori di stato–azione utilizzando
l’azione effettivamente selezionata nello stato successivo, il Q-Learning
stima i valori assumendo che l’agente adotti sempre un comportamento
ottimale nel futuro. Questa caratteristica rende l’algoritmo
particolarmente adatto a scenari online, in cui l’esplorazione è
necessaria ma non deve influenzare direttamente la stima della policy
ottimale. La demo implementata ha lo scopo di rendere osservabile in tempo
reale il processo decisionale dell’agente una volta completata la fase di
addestramento.

\subsubsection{Ambiente di simulazione}
L’ambiente di simulazione utilizzato per il Q-Learning coincide con quello
descritto nella sezione dedicata a SARSA. Tale scelta consente un confronto
diretto tra approcci \textit{on-policy} e \textit{off-policy}, eliminando
possibili fattori di confondimento dovuti a differenze ambientali.

L’ambiente è modellato come un processo decisionale di Markov e
implementa una variante del gioco del Blackjack con spazio delle azioni
limitato alle decisioni \textit{Hit} e \textit{Stand}. La funzione di
ricompensa adotta uno schema neutro, assegnando $+1$ in caso di vittoria,
$-1$ in caso di sconfitta e $0$ in caso di pareggio.

\subsubsection{Rappresentazione dello stato}
La rappresentazione dello stato è identica a quella adottata
nell’implementazione SARSA, ovvero:
\[
s = (\text{player\_sum}, \text{is\_soft}, \text{dealer\_showing})
\]
dove $\text{player\_sum}$ rappresenta la somma della mano del giocatore,
$\text{is\_soft}$ indica la presenza di un asso utilizzabile come valore
$11$ e $\text{dealer\_showing}$ è la carta scoperta del dealer.

Il mantenimento della medesima rappresentazione consente di preservare la
comparabilità tra le policy apprese dai due algoritmi, isolando l’effetto
della diversa regola di aggiornamento.


\subsubsection{Agente Q-Learning}
La struttura dell’agente Q-Learning ricalca quella dell’agente SARSA per
quanto riguarda la rappresentazione della funzione di valore d’azione
$Q(s,a)$, memorizzata in una Q-table implementata come dizionario annidato
e inizializzata implicitamente a zero per tutte le coppie stato–azione non
ancora visitate.

L’agente utilizza una strategia di esplorazione $\epsilon$-greedy durante
la fase di addestramento. Con probabilità $\epsilon$ viene selezionata
un’azione casuale, mentre con probabilità $1 - \epsilon$ viene scelta
l’azione con valore $Q$ massimo. Il parametro $\epsilon$ decresce
progressivamente secondo una legge di decadimento esponenziale,
favorendo inizialmente l’esplorazione e successivamente lo sfruttamento
delle conoscenze acquisite.

Analogamente a SARSA, nei casi in cui uno stato non sia presente nella
Q-table o non possieda azioni precedentemente valutate, l’agente seleziona
un’azione casuale, garantendo robustezza nelle fasi iniziali
dell’apprendimento. È inoltre introdotta una regola deterministica che
forza l’azione \textit{Stand} quando la somma della mano del giocatore è
maggiore o uguale a $21$, evitando transizioni non valide.

\subsubsection{Aggiornamento Q-Learning}
La principale differenza rispetto a SARSA risiede nella regola di
aggiornamento dei valori di stato–azione. Per ogni transizione osservata
$(s_t, a_t, r_t, s_{t+1})$, il valore viene aggiornato secondo la formula:
\[
Q(s_t, a_t) \leftarrow Q(s_t, a_t) +
\alpha \left[ r_t + \gamma \max_{a'} Q(s_{t+1}, a') - Q(s_t, a_t) \right]
\]

L’operatore $\max_{a'}$ seleziona il valore massimo tra tutte le azioni
disponibili nello stato successivo, indipendentemente dall’azione che
verrà effettivamente eseguita. Questo rende l’algoritmo \textit{off-policy}:
la \textit{target policy} rispetto alla quale vengono stimati i valori è
sempre la policy greedy ottimale, mentre la \textit{behavior policy}
utilizzata per interagire con l’ambiente è la policy $\epsilon$-greedy.

Nel caso di stati terminali, l’aggiornamento utilizza esclusivamente la
ricompensa osservata:
\[
Q(s_t, a_t) \leftarrow Q(s_t, a_t) +
\alpha \left[ r_t - Q(s_t, a_t) \right]
\]
...
Al termine di ogni episodio, il parametro $\epsilon$ viene aggiornato
secondo la medesima legge di decadimento adottata nell’implementazione
SARSA, garantendo coerenza nelle modalità di esplorazione.

\subsubsection{Procedura di Addestramento}
La procedura di addestramento ricalca quella adottata per SARSA, mantenendo
invariati il numero di episodi, i limiti sul numero massimo di passi per
episodio e la strategia di decadimento del parametro $\epsilon$. Tale
scelta consente di confrontare in modo diretto i due algoritmi,
attribuendo eventuali differenze di comportamento esclusivamente alla
diversa natura \textit{on-policy} e \textit{off-policy}.

L’addestramento viene eseguito su $500{,}000$ episodi e viene condotto in
un thread separato al fine di preservare la responsività dell’interfaccia
grafica. Durante il training, l’agente aggiorna la Q-table ad ogni
transizione osservata secondo la regola del Q-Learning.

\subsubsection{Valutazione della Policy}
Al termine dell’addestramento, la demo attiva una modalità di gioco
automatica in cui l’agente opera secondo una policy effettivamente greedy,
senza esplorazione. Per ogni stato raggiunto, il sistema visualizza in
tempo reale i valori $Q(s,\textit{Stand})$ e $Q(s,\textit{Hit})$ appresi
durante il training, rendendo esplicito il processo decisionale
dell’agente.

Le decisioni vengono prese con intervalli temporali di cinque secondi,
consentendo di osservare il processo di inferenza in modo chiaro e
intuitivo. Al termine di ciascun episodio, una nuova mano viene avviata
automaticamente, permettendo l’osservazione continua del comportamento
della policy appresa.

\subsection{Analisi della Policy – Q-Learning Online}

La policy appresa dall’agente Q-Learning online è stata analizzata sia dal
punto di vista quantitativo che qualitativo. A differenza dell’approccio
adottato per SARSA, in cui la valutazione viene condotta in una fase
separata disabilitando l’esplorazione, nel Q-Learning l’analisi delle
prestazioni è integrata nel processo di addestramento, sfruttando metriche
cumulative e strumenti di visualizzazione della policy finale.

L’andamento della ricompensa media nel corso degli episodi mostra una
progressiva stabilizzazione, indicando la convergenza del processo di
apprendimento. In particolare, la reward media tende a valori prossimi allo
zero, coerentemente con l’adozione di una funzione di ricompensa neutra,
che assegna $+1$ alle vittorie, $-1$ alle sconfitte e $0$ ai pareggi.

Un’ulteriore conferma della convergenza è fornita dall’analisi del win rate
cumulativo, che cresce gradualmente nel corso dell’addestramento e tende
ad avvicinarsi ai valori attesi dalla \textit{basic strategy} del Blackjack.
Questo comportamento evidenzia la capacità del Q-Learning di apprendere
una policy prossima a quella ottimale pur mantenendo una strategia di
esplorazione durante il training, caratteristica tipica degli algoritmi
\textit{off-policy}.

La policy finale è stata inoltre visualizzata tramite heatmap separate per
mani \textit{hard} e \textit{soft}. Per ogni combinazione di somma del
giocatore e carta scoperta del dealer, le mappe indicano l’azione preferita
dall’agente. Le policy risultanti mostrano una forte coerenza con la
strategia ottimale teorica, in particolare nelle regioni dello spazio degli
stati più frequentemente visitate. Le discrepanze osservate sono
principalmente localizzate in stati rari o marginali, dove la frequenza di
visita durante il training risulta inferiore.

Nel complesso, l’analisi evidenzia come il Q-Learning online sia in grado di
apprendere una policy stabile e competitiva, confermando l’efficacia
dell’ap\-proccio off-policy nel contesto del Blackjack e giustificando le
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{SARSAonlineResult.png}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{SARSArewardDistribution.png}
    \caption{Distribuzione dei reward – Online SARSA}
    \label{fig:SARSAonlineResult2}
\end{figure}

%bisogna spostare le figure sopra
...
Dopo tutte le analisi e le considerazioni effettuate, la decisione di implementare un prototipo del nostro agente non poteva che ricadere sui due modelli più performanti e che sono riusciti a raggiungere (o quasi) la base strategy per massimizzare il win rate. Abbiamo quindi implementato un 
