...
\title{Soft17 -- An AI Agent for Blackjack}
...
\fancyfoot[C]{\thepage}
\pagestyle{fancy}
\fancyhead[L]{\includegraphics[height=1.2cm]{informatica.jpg}}
\fancyhead[R]{
 \raggedleft 
 \footnotesize
...
\includegraphics[width=8cm]{logo-unisa.png}\\[0.5cm]

{\Large Università degli Studi di Salerno}\\[0.3cm]
{\large Dipartimento di Informatica}\\[0.3cm]
{\large Laurea Triennale in Informatica}\\[0.3cm]
{\large Corso di Fondamenti di Intelligenza Artificiale}\\[0.5cm]
{\Huge\textbf{Soft17 -- An AI Agent for Blackjack}}\\[0.3cm]
\makebox[\textwidth]{\centering
  \includegraphics[width=6cm]{logo-soft17.png}
}\\
{\large Abbatiello Simone \\
0512119659 \\
Nappi Vincenzo \\
0512119813\\
Niemiec Francesco\\
...
Guidati dalla curiosità e dalla relativa semplicità delle regole, abbiamo quindi scelto il gioco del \textbf{Blackjack}. Da questa scelta nasce l’idea di sviluppare un modello che sia in grado di stimare l’esito atteso di una mano di Blackjack sulla base delle informazioni disponibili e sulla base di tutto ciò compiere decisioni sensate e giocare in maniera autonoma.
...
Per ciascuna combinazione di mano del giocatore e carta visibile del banco, viene determinata l’azione che massimizza il valore atteso della giocata. È importante effettuare una distinzione tra \textbf{hard-hand}, in cui l’asso ha valore fisso pari a 1, e \textbf{soft-hand}, in cui l’asso può assumere valore 11 senza rischio immediato di sballo.
...
\begin{center}
      \includegraphics[scale=0.38, center]{img/BaseStrategy.png}
  \end{center}

\section{Tecnologie utilizzate}
\subsection{Overview}
Abbiamo deciso di sfruttare l'ambiente condiviso online \textbf{Google Colab} e di conseguenza il linguaggio scelto per raggiungere l'obiettivo prefissato è \textbf{python}. In particolare abbiamo utilizzato le seguenti librerie: 
\begin{itemize}
    \item \textbf{pandas}: permette l'esplorazione a la manipolazione di dati in modo semplice e flessibile;
    \item \textbf{numpy}: ottimizza la gestione di ampi dati multidimensionali come array e matrici in modo efficiente e fornisce una serie di funzioni matematiche per la loro gestione;
    \item \textbf{seaborn e matplotlib}: permettono la visualizzazione di dati tramite schemi o grafici informativi. Sono state principalmente utilizzata per mostrare i risultati dei training degli algoritmi implementati.
    \item \textbf{scikit-learn}: fornisce tool semplici ed efficienti che permettono la realizzazione di algoritmi di machine learning. Permette inoltre di valutare il modello con il calcolo delle metriche integrato. 
    \item \textbf{tkinker}: permette di sviluppare interfacce grafiche per i programmi python implementati.
\end{itemize}
...
\begin{description}
    ...
    \item[Actuators:] gli attuatori dell’agente corrispondono alle azioni disponibili: Hit, Stand.
...
\subsection{Scelta della prima strategia}
...
\begin{enumerate}
    ...
    \item Utilizzare dataset già disponibili online \footnote{(\url{https://www.kaggle.com/datasets/mojocolors/900000-hands-of-blackjack-results})}\footnote{(\url{https://www.kaggle.com/datasets/dennisho/blackjack-hands/data})}.
\end{enumerate}
La prima opzione come pro avrebbe garantito un grande controllo sui dati, ma un errore minimo nel simulatore o una nostra scelta sbagliata e/o inesperta nella categorizzazione dei dati avrebbe avuto un impatto troppo alto sul modello.
...
Avendo valutato per bene ambo le alternativi ci siamo trovati inizialmente di fronte ad un bivio, optando poi per una soluzione "ibrida".
...
Abbiamo quindi preso questo programma e generato un dataset con circa un milione di mani giocate, una quantità tale da poter effettuare un buon training in tempi tutto sommato contenuti.
È importante notare come la scelta di mantenere inalterato l'algoritmo nasce da due ragioni: innanzitutto per mantenere inalterati gli ottimi risultati relativi all'usabilità del dataset, la seconda (nonchè forse la più importante) è quella di voler andare ad operare direttamente sui dati anzichè sulla loro generazione.
...
Iniziamo però da una fase iniziale di esplorazione dei dati, la quale serve per comprendere su quali aree agire.
...
Come prima cosa quindi abbiamo importato il file del dataset (in formato .csv) per visionare le caratteristiche. In particolare il dataset iniziale era composto da: 
\begin{itemize}
...
\begin{itemize}
    \item 421940 istanze con win \textbf{maggiore di zero.}
    \item 493992 istanze con win \textbf{minore di zero.}
    \item 84068 istanze con win \textbf{uguale a zero.}
\end{itemize}
...
\begin{description}
    ...
    \end{description}
Abbiamo quindi optato per una strategia di discretizzazione con soglie che ci permette di fare esattamente ciò che
ci serve.
...
\subsection{Modeling}
Prima di parlare degli algoritmi utilizzati occorre fare delle premesse. Per scegliere i migliori parametri adatti agli algoritmi e il problema che stiamo affrontando abbiamo deciso di utilizzare una \textbf{griglia di ricerca} per comprendere i valori adatti per i vari iperparametri. In particolare \textbf{GridSearchCV}, una classe contenuta nella libreria scikit-learn che ci permette di testare più combinazioni di parametri automaticamente evitando di rieseguire manualmente i training (e sfrutta cross-validation).
...
Infine \textit{stratify} permette di suddividere il dataset in modo che ogni sottoinsieme mantenga la stessa identica proporzione di classi del dataset originale.
...
I migliori parametri scelti dalla griglia sono: max\_depth = 10, min\_samples\_leaf= 1, min\_samples\_split= 2.\newline Le metriche invece ci hanno fornito i seguenti risultati: 
\begin{itemize}
    \item F1-Score Test: 0.6627
    \item Accuracy:  0.6871
    \item Precision:  0.7291
    \item Recall:  0.6073
\end{itemize}
  ...
  \begin{center}
      \includegraphics[scale=1.0, center]{img/ConfusionMatrixPipeline1A.png}
  \end{center}
...
pipeline 1A, al fine di garantire un confronto equo tra i modelli. \newline Analogamente a quanto fatto per la pipeline 1A, anche in questo caso è stato
...
\begin{center}
      \includegraphics[scale=0.75, center]{img/ConfusionMatrixPipeline1B.png}
\end{center}
...
\begin{itemize}
    \item il Blackjack è un \textbf{processo decisionale sequenziale}, in cui ogni azione influenza gli stati futuri della partita, tuttavia ciò non viene riflesso con questi approcci;
    \item l'assenza di feedback intermedio;
    \item l’azione ritenuta migliore viene dedotta indirettamente a partire dalle predizioni del finale delle partite del modello:
...
Abbiamo quindi identificato un facile parallelismo col blackjack, ma con nozioni veramente basilari non sarebbe stato possibile andare avanti. Abbiamo quindi condotto delle ricerche basilari, necessarie per capire se stessimo ancora sbagliando strada. 
...
\newline Un Processo di Ricompensa di Markov è definito come la quadrupla:
\[
...
\]
L'obiettivo è quello di trovare una policy ottimale, ovvero una strategia che massimizza la ricompensa cumulativa a lungo termine, e che quindi rispetto alle altre policy produrrà un risultato almeno uguale.
Definiamo in particolare un \textbf{MDP Parzialmente Osservabile} (POMDP) un processo decisionale di Markov con stati nascosti, ovvero composto dalla 7-tupla:
\[
...
Una volta classificato il problema, abbiamo deciso di adottare un approccio di \textbf{Reinforcement Learning}, che viene naturale siccome trattiamo il tutto come un MDP.
...
\begin{center}
      \includegraphics[scale=0.6, center]{img/agent.png}
...
possono risultare rischiose in fase di esplorazione.
Vale la pena osservare ambo gli approcci, per comprenderne al meglio i trade-off che si ottengono applicando uno rispetto all'altro nel caso del Blackjack.
Abbiamo quindi selezionato due algoritmi:
\begin{description}
    \item[- Q-learning:] che è un algoritmo di tipo off-policy.
    \item[- Sarsa:] che è un algoritmo di tipo on-policy.
\end{description}
Abbiamo deciso di partire dal primo di questi.
\section{Q-learning}
Il \textbf{Q-learning} è un algoritmo di Reinforcement Learning \textit{off-policy}, introdotto da Christopher J. C. Watkins nel 1989 \cite{watkins1989learning}, con l’obiettivo di apprendere una policy ottimale
...
Questa caratteristica consente all’algoritmo di convergere verso una policy ottimale anche in presenza di comportamenti esplorativi rischiosi.
\section{SARSA}
...
SARSA fa la sua prima apparizione nel paper "On-Line Q-Learning Using Connectionist System"\cite{SARSA}, sotto il nome di "Modified Connectionist Q-Learning". L'algoritmo ha due idee cardine dietro al suo funzionamento:
\begin{description}
...
Occorre infine definire come un'ultima distinzione, fondamentale per comprendere le nostre implementazioni, la differenza tra \textit{online} ed \textit{offline} reinforcement learning.
\textbf{L'online reinforcement learning} prevede di simulare interazioni con l'ambiente per apprendere delle policy ottimali. Utilizza di volta in volta l'esperienza immediatamente appresa nelle successive interazioni con l'ambiente ed ha il vantaggio di potersi adattare ai cambiamenti date le continue interazioni con esso. Il grande problema di questo approccio è che a causa di tutte le simulazioni che genera con l'ambiente è computazionalmente costoso. Di fatto quindi questo tipo di approccio non necessità di un dataset per l'addestramento.
A differenza del precedente \textbf{l'offline reinforcement learning} mira a creare delle policy efficaci su dati collezionati precedentemente, senza nessuna interazione con l'ambiente. Questo approccio porta con se anche delle sfide importanti: occorre migliorare le policy che si raccolgono dai dati così da stimare valori per azioni non presenti nel dataset.
...
\begin{itemize}
    ...
    \item $\gamma$: termine che regola come comportarsi nel corso della ricerca della soluzione. Fa in modo che più vicino si arriva alla soluzione, maggiore sarà la preferenza verso dei reward a breve termine. Definisce quanto è lungimirante un agente ed anche questo varia tra 0 e 1.
    \item $\epsilon$: termine che regola l'esplorazione causandone una diminuzione nel tempo di modo che la policy appresa migliori sfuttando le conoscenze acquisite nel tempo.
\end{itemize}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{img/3a.png}
...
risulta costosa, rischiosa o impraticabile \cite{levine2021offline}. \newline Nel contesto del presente progetto, l'approccio offline è stato introdotto con l'obiettivo di investigare se un agente possa apprendere una politica
...
convergenza dell'algoritmo e sulle prestazioni finali della politica appresa.\newline Recenti lavori in letteratura hanno evidenziato come l'ottimizzazione
...
per la selezione delle azioni del giocatore.\newline La modifica fondamentale apportata al codice di generazione riguarda la
...
\newline In primo luogo, la limitazione dello spazio delle azioni a due sole opzioni
...
modificato è una politica semplificata: il giocatore richiede
...
transizioni sufficientemente variegate per l'apprendimento offline. \newline In secondo luogo, la semplificazione della strategia di generazione dei
...
robustezza rispetto alla distribuzione dei dati.\newline Il dataset finale contiene un milione di transizioni, ciascuna
...
l'addestramento mediante Q-learning offline.\newline È importante sottolineare che il codice modificato per la generazione del
...
\end{equation} \newline Questa regola di aggiornamento è formalmente identica a quella utilizzata
...
selezionando in ogni stato l'azione con valore Q massimo. \newline I risultati sperimentali mostrano un \textit{win rate} pari a 0.3814 (38.14\%),
un \textit{loss rate} di 0.5666 (56.66\%) e un \textit{draw rate} di 0.0520 (5.20\%).
...
performante rispetto all'approccio online.\newline L'analisi delle cause di tale discrepanza evidenzia due fattori
...
nell'offline reinforcement learning.\newline In secondo luogo, l'assenza di esplorazione attiva durante
...
riproducibilità e riutilizzo dei dati.\newline     Tali osservazioni sono coerenti con risultati recenti che mostrano come
...
\textit{off-policy evaluation problem}.\newline La letteratura recente propone estensioni del Q-learning offline, come
...
nel contesto del Blackjack rappresenta un divario non trascurabile.
\newline L'analisi della distribuzione degli esiti rivela ulteriori differenze qualitative. 
...
Al contrario, la politica offline, influenzata dalla strategia del dataset che richiede carta fino a 17, 
tende ad assumere maggiori rischi, risultando in un numero inferiore di pareggi ma anche in un \textit{loss rate} 
più elevato (56.66\% contro 51.04\%). \newline Dal punto di vista della stabilità e convergenza, l'implementazione online beneficia del meccanismo di esplorazione 
...
un utilizzo più efficiente delle risorse di calcolo.\newline Nel contesto specifico del presente progetto, il beneficio computazionale
...
risulta marginale nel caso in esame.\newline La situazione cambia in modo sostanziale quando il dataset è già disponibile
...
di transizioni se i dati sono ben distribuiti e rappresentativi.\newline Un terzo trade-off concerne la robustezza rispetto a errori di stima. L'approccio online beneficia del meccanismo di correzione continua
...
invece, può accumulare errori di estrapolazione in regioni dello spazio degli stati scarsamente rappresentate nel dataset, 
...
requisiti specifici del progetto e gli obiettivi di apprendimento prefissati.\newline In primo luogo, le prestazioni superiori della l'approccio online rappresentano il criterio decisionale primario. 
...
\newline In secondo luogo, l'autonomia e la flessibilità dell'approccio online si allineano meglio con gli obiettivi didattici 
...
obiettivi formativi basilari del corso. \newline In terzo luogo, la maggiore robustezza e affidabilità della versione online la rendono più adatta per un sistema che potrebbe 
...
\newline È tuttavia importante sottolineare che l'approccio offline ha dimostrato potenzialità significative e 
...
\newline In conclusione, la scelta dell'approccio online riflette 
...
Nei casi in cui uno stato non sia presente nella Q-table o non possieda
azioni precedentemente valutate, l’agente seleziona un’azione casuale,
garantendo robustezza anche nelle fasi iniziali dell’apprendimento.
È inoltre introdotta una regola deterministica che forza l’azione \textit{STAND} quando la somma della mano del giocatore è maggiore o uguale a 21, evitando transizioni non valide e riducendo il rumore nell’aggiornamento
dei valori Q.
\subsubsection{Aggiornamento SARSA}
Per ogni transizione osservata $(s_t, a_t, r_t, s_{t+1}, a_{t+1})$,
il valore viene aggiornato secondo la regola:
\[
Q(s_t, a_t) \leftarrow Q(s_t, a_t) +
...
\]
Al termine di ogni episodio, il parametro $\epsilon$ viene aggiornato
secondo una legge di decadimento esponenziale:
\[
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{img/SARSApolicy.png}
    \caption{SARSA Policy – Online SARSA}
    \label{fig:SARSAonlineResult2}
\end{figure}

\subsection{Implementazione Q-Learning Online}

Dopo aver descritto l’implementazione dell’algoritmo SARSA, basato su un
approccio \textit{on-policy}, viene ora introdotto il Q-Learning online,
algoritmo appartenente alla famiglia \textit{off-policy}. L’obiettivo di
questa sezione è analizzare l’impatto della separazione tra
\textit{behavior policy} e \textit{target policy}, mantenendo invariati gli
altri elementi progettuali al fine di garantire un confronto diretto e
significativo.
L’ambiente di simulazione, la rappresentazione dello stato, la struttura
dell’agente, la strategia di esplorazione e la procedura di addestramento
rimangono identici a quanto descritto nella sezione dedicata a SARSA.
Questa scelta progettuale consente di isolare l’effetto della diversa
regola di aggiornamento dei valori di stato–azione, che rappresenta
l’unica reale differenza tra i due algoritmi considerati.
Al termine dell’addestramento, l’agente entra in una fase di esecuzione
completamente autonoma, durante la quale gioca automaticamente seguendo
una policy greedy derivata dalla Q-table appresa. In questa modalità di
demo, l’agente seleziona le azioni senza esplorazione e senza intervento
esterno, consentendo di osservare in tempo reale il comportamento emergente
della policy appresa e il processo decisionale associato.


\subsubsection{Aggiornamento Q-Learning}

L’aggiornamento dei valori di stato–azione nel Q-Learning rappresenta la
principale differenza rispetto all’algoritmo SARSA. Mentre in SARSA
l’aggior\-namento avviene in modo \textit{on-policy}, utilizzando l’azione
effettivamente selezionata dalla policy corrente nello stato successivo,
il Q-Learning adotta un approccio \textit{off-policy}, stimando
direttamente i valori della policy ottimale.

Per ogni transizione osservata $(s_t, a_t, r_t, s_{t+1})$, il valore
$Q(s_t, a_t)$ viene aggiornato assumendo che, nello stato successivo,
l’agente selezioni sempre l’azione con valore $Q$ massimo. In questo modo,
l’aggiornamento risulta indipendente dalla strategia di esplorazione
utilizzata durante l’interazione con l’ambiente e consente di apprendere
una stima della \textit{target policy} greedy anche in presenza di azioni
esplorative.

Questa differenza rende il Q-Learning particolarmente adatto a scenari di
apprendimento online, in cui l’esplorazione è necessaria per visitare lo
spazio stato–azione, ma non deve influenzare direttamente la valutazione
dei valori appresi, a differenza di quanto avviene nell’approccio SARSA.

\subsection{Risultati sperimentali e confronto con Q-Learning online}
L'implementazione con \textbf{SARSA online} si è dimostrata ancora più efficace dell'implementazione con il Q-Learning online. Al termine del training su cinquecentomila episodi, la Q-table risultante contiene valori appresi per 280 azioni coppie-stato. I risultati finali hanno un \textit{win rate} del 42.67\%, contro il 40.29\% ottenuto con il Q-Learning online ed un miglioramento di più di 4 punti percentuale rispetto al Q-Learning offline.
Si può osservare un leggero aumento del \textit{draw rate} che raggiunge il 9.43\% rispetto all '8.67\% dell'approccio con Q-Learning online. Infine si ha una significativa riduzione delle sconfitte che raggiungono il 47.90\%, il risultato più basso ottenuto tra tutti gli approcci analizzati. Nonostante il grande traguardo del limite di vittorie teorico raggiunto, anche questa implementazione ha gli stessi compromessi analizzati durante l'implementazione dell'online Q-Learning.
Nelle seguenti figure è possibile osservare il progredire del modello fino a raggiungere risultati vicini alla \textit{basic strategy} inizialmente illustrata.


\begin{figure}[!ht]
    \centering
    \includegraphics[width=1.00\textwidth]{img/SARSAonlineResult.png}
    \caption{Risultati Implementazione – Online SARSA}
    \label{fig:SARSAonlineResult}
\end{figure}


\section{Confronto finale tra le implementazioni di Reinforcement Learning}
...
\begin{table}[h]
    \centering
    \caption{Confronto delle prestazioni degli agenti RL su Blackjack}
    \label{tab:rl-comparison}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Algoritmo} & \textbf{Win Rate (\%)} & \textbf{Loss Rate (\%)} & \textbf{Draw Rate (\%)} \\
        \midrule
...
\section{Considerazioni finali e Demo implementate}
Dopo tutte le analisi e le considerazioni effettuate, la decisione di implementare un prototipo del nostro agente non poteva che ricadere sui due modelli più performanti che sono riusciti a raggiungere (o quasi) la base strategy per massimizzare il \textbf{win rate}. Abbiamo quindi implementato una demo con il Q-Learning online e il SARSA online. Le demo sono di fatto uguali con l'unica differenza è che quella del Q-Learning gioca automaticamente mani di blackjack mentre l'implementazione con SARSA prevede una demo interattiva. La demo è stata implementata con la libreria python  \texttt{tkinker} che permette di realizzare interfacce grafiche.
La demo permette quindi di visualizzare graficamente il banco e le carte in gioco sulla parte sinistra, mentre permette di visualizzare informazioni utili e le predizioni del modello nella parte a destra. La demo interattiva con SARSA include tre tasti: il primo permette di giocare una partita, il secondo di fare hit ed il terzo permette di fare stand. A destra sarà possibile visualizzare la scelta del modello consigliata per prendere le decisioni secondo la policy appresa dal modello.\newline Con riferimento alla demo che gioca automaticamente con Q-Learning le uniche differenze sono i tasti disattivati, non è quindi possibile interagire con il gioco in esecuzione.
