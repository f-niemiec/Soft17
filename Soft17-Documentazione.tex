...
\subsection{Agente SARSA}
L’agente viene inizializzato con i seguenti iperparametri:
\begin{itemize}
    \item $\alpha$ (\textit{learning rate}), che controlla la velocità di aggiornamento dei valori Q;
    \item $\gamma$ (\textit{discount factor}), che determina l’importanza delle ricompense future;
    \item $\epsilon$, parametro iniziale della strategia di esplorazione $\epsilon$-greedy;
    \item $\epsilon_{\text{decay}}$, fattore di decadimento esponenziale di $\epsilon$;
    \item $\epsilon_{\text{min}}$, valore minimo consentito per l’esplorazione.
\end{itemize}
